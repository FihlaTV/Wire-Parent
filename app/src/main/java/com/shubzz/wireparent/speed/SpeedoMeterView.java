package com.shubzz.wireparent.speed;
...
public class SpeedoMeterView extends View {
    ...
    public Bitmap transform(Bitmap source) {
        source = Bitmap.createScaledBitmap(source, 500, 500, false);
    ...
    private void createSpeedometerDisk() {
        ...
        backgroundbitmap = Bitmap.createBitmap(500, 500, Bitmap.Config.ARGB_4444);
        ...
        if (isborder)
            canvas.drawCircle(centerx, centerx, radius - (strokewidth + (strokewidth / 2.0f)), paint);
        paint.setShader(null);
        paint.setColor(Color.rgb(35, 35, 35));
        paint.setStyle(Paint.Style.FILL);
        if (backimagebitmap == null)
            canvas.drawCircle(centerx, centerx, radius - (strokewidth * 2.0f), paint);
        paint.setStyle(Paint.Style.STROKE);
...
//        canvas.drawArc(new RectF(80,80,getWidth()-80,getHeight()-80),125,180,false,paint);
    ...
    public void setSpeed(int speed, boolean wantmaintainspeed) {
        ...
        speedAnimator.setDuration(20000);
